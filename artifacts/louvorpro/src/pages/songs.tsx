...
import { Music, Music2, Plus, Search, ExternalLink, Pencil, Trash2, Sparkles, MoreVertical, BarChart2, Flame, Snowflake, TrendingUp } from "lucide-react";
        ...
        <Dialog open={healthOpen} onOpenChange={setHealthOpen}>
          <DialogContent className="sm:max-w-[520px] max-h-[90vh] flex flex-col">
            <DialogHeader>
              ...
              <p className="text-xs text-muted-foreground">Baseado no histórico de cultos realizados</p>
            </DialogHeader>
                ...
                <>
                  ...
                  <div>
                    <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground flex items-center gap-1.5 mb-3">
                      ...
                      Top 5 Mais Tocadas — últimos 90 dias
                    </p>
                    {!healthScore?.topPlayed.length ? (
                      <div className="text-sm text-muted-foreground py-4 text-center border border-dashed border-border/50 rounded-lg">
                        Nenhum culto registrado nos últimos 90 dias.
                      </div>
                    ) : (
                      <div className="space-y-2">
                        {healthScore.topPlayed.map(({ song, count }, i) => {
                          const maxCount = healthScore.topPlayed[0].count;
                          const pct = Math.round((count / maxCount) * 100);
                          return (
                            <div key={song.id} className="flex items-center gap-3">
                              <span className="text-xs font-bold text-muted-foreground/60 w-4 text-right flex-shrink-0">{i + 1}</span>
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center justify-between gap-2 mb-1">
                                  <p className="text-sm font-medium text-foreground truncate">{song.title}</p>
                                  <span className="text-xs font-bold text-primary flex-shrink-0 flex items-center gap-1">
                                    <TrendingUp className="w-3 h-3" />{count}x
                                  </span>
                                </div>
                  ...
                  {/* Geladeira */}
                  <div>
                    <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground flex items-center gap-1.5 mb-1">
                      <Snowflake className="w-3.5 h-3.5 text-sky-400" />
                      Na Geladeira — sem tocar há 60+ dias
                    </p>
                    <p className="text-[11px] text-muted-foreground mb-3">
                      {healthScore?.frozen.length ?? 0} músicas do banco que não foram escaladas recentemente.
                    </p>
                    {!healthScore?.frozen.length ? (
                      <div className="text-sm text-muted-foreground py-4 text-center border border-dashed border-border/50 rounded-lg">
                        Todas as músicas foram tocadas recentemente. 
                      </div>
                    ) : (
                      <div className="rounded-lg border border-border/50 divide-y divide-border/30 max-h-56 overflow-y-auto">
                        {healthScore.frozen.map(song => (
                          <div key={song.id} className="flex items-center gap-3 px-3 py-2">
                            <Snowflake className="w-3 h-3 text-sky-400/60 flex-shrink-0" />
                            <div className="min-w-0 flex-1">
                              <p className="text-sm font-medium text-foreground truncate">{song.title}</p>
                              <p className="text-[11px] text-muted-foreground truncate">{song.artist || "—"}{song.key ? ` • Tom: ${song.key}` : ""}</p>
                            </div>
                          </div>
                        ))}
                      </div>
            ...
            <DialogFooter className="pt-2">
              <Button variant="outline" onClick={() => setHealthOpen(false)}>Fechar</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
